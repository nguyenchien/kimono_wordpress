...
class WPCF7_ContactForm {
	...
	public function form_html( $args = '' ) {
        ...
        /*BEGIN TriLe - Call javascript after submit*/
        $submitedArray = null;
        /*END TriLe - Call javascript after submit*/
		if ( $this->is_posted() ) {
			$submission = WPCF7_Submission::get_instance();
            $returnSubmission = '';
			if ( $submission->is( 'validation_failed' ) ) {
				$class .= ' invalid';
                $returnSubmission = 'validation_failed';
			} elseif ( $submission->is( 'spam' ) ) {
				$class .= ' spam';
                $returnSubmission = 'spam';
			} elseif ( $submission->is( 'mail_sent' ) ) {
				$class .= ' sent';
                $returnSubmission = 'mail_sent';
			} elseif ( $submission->is( 'mail_failed' ) ) {
				$class .= ' failed';
                $returnSubmission = 'mail_failed';
			}
                $submitedArray = $submission->get_posted_data();
                $submitedArray['submited_status'] = $returnSubmission;
                $submitedArray['submited_form_name'] = $name_attr;
                //Yii::log("submited_status = $returnSubmission", CLogger::LEVEL_INFO, 'wedebug');
		}
        ...
        /*BEGIN TriLe - Call javascript after submit*/
        if (count($submitedArray) > 0) {
            $js = array();
            $js[] = '<script type="text/javascript">';
            $js[] = 'wpcf7FormSubmited = '. CJSON::encode($submitedArray) .';';
            $js[] = '</script>';
            $html .= implode('', $js);
        }
        /*END TriLe - Call javascript after submit*/
