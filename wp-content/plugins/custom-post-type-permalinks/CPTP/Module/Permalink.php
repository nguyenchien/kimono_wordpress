	...
	private function create_taxonomy_replace_tag( $post_id, $permalink ) {
		...
		foreach ( $taxonomies as $taxonomy => $objects ) {
			...
			if ( false !== strpos( $permalink, '%' . $taxonomy . '%' ) ) {
                ...
                // Duyen customize code for selected term to show in post link
                $primary_term = !empty($primary_term) ? $primary_term : get_post_meta($post_id, '_category_permalink', true);
                if($primary_term){
                    $term_obj = get_term($primary_term, $taxonomy);
