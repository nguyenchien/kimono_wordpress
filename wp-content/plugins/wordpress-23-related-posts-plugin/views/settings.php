	...
	<form method="post" enctype="multipart/form-data" action="<?php echo $form_url; ?>" id="wp_rp_settings_form" style="display: <?php echo $form_display; ?>;">
		...
		<div id="wp_rp_advanced_settings_collapsible" block="advanced_settings" class="settings_block collapsible collapsed">
			...
			<div class="container" style="display: none">
				...
				<h3><?php _e("Related post with custom post type",'wp_rp_related_custom_post_type'); ?></h3>
				<div>
					<table class="form-table">
						<tr valign="top">
							<th scope="row"><?php _e("Enable post types",'wp_rp_related_custom_post_type'); ?></th>
							<td>
								<div class="post-types">
									<?
									$post_types_current_used = !empty($options['post_types']) ? explode(',', $options['post_types']): array();
									$post_types = get_post_types();
									
									foreach ($post_types as $pt):
										?>
										<label>
											<input name="wp_rp_post_types[]" type="checkbox" id="wp_rp_post_types" value="<?php esc_attr_e($pt); ?>"<?php checked(in_array($pt, $post_types_current_used)); ?> />
											<?php esc_html_e($pt); ?>
											<br />
										</label>
									<?php endforeach; ?>
