...
//	if ($post->post_type !== 'post' ||  $post->post_status !== 'publish') {
	if ($post->post_status !== 'publish') { // Tudh Modified in case for custom post_type
		return;
	}
...
function wp_rp_fetch_related_posts_v2($limit = 10, $exclude_ids = array()) {
	...
	$post_query = $wpdb->prepare("
		...
		WHERE post.ID IN (" . implode(', ', array_fill(0, count($related_post_ids), '%d')) . ")
			AND post.post_type = '$post->post_type'
...
function wp_rp_fetch_random_posts($limit = 10, $exclude_ids = array()) {
	...
	$q1 = "SELECT ID FROM $wpdb->posts posts WHERE post_status = 'publish' AND post_type = '$post->post_type' AND ID NOT IN ($exclude_ids_str)";
	if($options['exclude_categories']) {
	...
	$q2 = "SELECT ID, post_title, post_content, post_excerpt, post_date, post_type , comment_count FROM $wpdb->posts WHERE post_status = 'publish' AND post_type = '$post->post_type' AND ID IN (" . implode(',', $ids) . ")";
...
function wp_rp_get_post_categories() {
	...
	$post_query = "SELECT p.ID as pid, te.name as cat_name, te.term_id as cat_id
		...
		WHERE p.post_status = 'publish' AND p.post_type = '$post->post_type'
