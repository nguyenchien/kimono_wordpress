...
function wp_rp_settings_page() {
	...
	if(sizeof($_POST)) {
		...
		// Tudh custom add list all post type
		
		if(!isset($postdata['wp_rp_post_types'])) {
			$new_options['post_types'] = '';
		} else if(is_array($postdata['wp_rp_post_types'])) {
			$new_options['post_types'] = join(',', $postdata['wp_rp_post_types']);
		} else {
			$new_options['post_types'] = trim($postdata['wp_rp_post_types']);
		}
