...
class HkPermaCat extends HkPermaCat_HkTools
{
	...
	public function insertPostJS()
	{
		if (!$this->isScategoryActivated) {
			...
			echo "<script type=\"text/javascript\">jQuery('#" . $taxonomy . "div').sCategoryPermalink({current: '$categoryID', taxonomy:'$taxonomy'});</script>\n";
	...
	public function savePost($new_status, $old_status, $post)
	{
		...
		$category_permalink = isset($_POST['scategory_permalink']) ? $_POST['scategory_permalink'] : "";
		$taxonomies = get_post_taxonomies();
		if(empty($taxonomies)){
			return false;
		}
		...
		if (isset($category_permalink)) {
			$cats = $this->wp_get_post_categories($post->ID, $taxonomy);
			foreach ($cats as $cid) {
		...
		if ($has_legacy_tag && $is_WP3) {
			...
			if ($this->hasNeedle($_SERVER['REQUEST_URI'], "options-permalink.php")) {
				...
				<div class="updated fade">
					<p>Your current Permalink Structure is <em><?php echo $permastruct; ?></em>, you are safe to replace
						it to <strong><?php echo $newPermastruct; ?></strong>.</p>
				</div>
			<?php } else {
				?>
				<div class="updated fade">
					<p><strong>Hikari Category Permalink</strong> plugin noted you are using the legacy permalink tag
						<em>%scategory%</em>. This tag is not required anymore, and custom categories can be used in
						posts permalinks with the default <em>%category%</em> tag.</p>
					<p>If you are using Wordpress 3.0 or newer and have no specific reason to use
						<em>%<strong>s</strong>category%</em>, please refere to your <a
							href="<?php echo $permalink_admin; ?>">Permalink admin page</a> and update your Permalink
						Structure.</p>
				</div>
			<?php }
		}
...
//					&& !in_array($post->post_status, array('draft', 'pending'))
	 ...
	 * @return array
	 */
	private function wp_get_post_categories($post_id = 0, $taxonomy = 'category', $args = array())
	{
		$post_id = (int)$post_id;
		...
		$cats = wp_get_object_terms($post_id, $taxonomy, $args);
		return $cats;
	}
