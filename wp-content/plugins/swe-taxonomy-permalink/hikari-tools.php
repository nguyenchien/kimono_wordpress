...
//	protected function __clone(){ return parent::__clone(); }
	...
	public function optionsHead()
	{
		...
		<style type="text/css">
			...
			.hikari-sidebar .widget {
				background: #FFF;
				border: solid 1px #DDD;
				margin-bottom: 10px;
			}

			.hikari-sidebar .title {
				background: #DBDBB6;
			...
			.hikari-uninstall-form {
				margin-top: 3em;
			}

			.hikari-uninstall-table {
				clear: none;
				width: 80%;
			}
		</style>
		...
		switch ($options_item['type']) {
			case 'text':
				...
				<tr>
					...
					<td>
						<input type="<?php echo $options_item['type']; ?>"
						       size="<?php echo $options_item['options']['size']; ?>" <?php echo $nameid;
						echo $tx_width; ?>
						       value="<?php echo htmlspecialchars($database_values[$options_item['id']]); ?>"/>
			...
			case 'textarea':
				...
				<tr>
					<?php echo $th_with_for; ?>
					<td>
						<textarea <?php echo $nameid;
						echo $ta_width; ?> cols="<?php echo $ta_options['cols']; ?>"
						                   rows="<?php echo $ta_options['rows']; ?>"><?php echo htmlspecialchars($ta_value); ?></textarea>
					...
					<td>
						<select <?php echo $nameid; ?>>
							...
							foreach ($options_item['options'] as $key => $option) {
								?>
								<option
									value="<?php echo $key; ?>" <?php selected($select_setting, $key); ?>><?php echo $option; ?></option>
							<?php } // foreach
							?>
				...
				<tr>
					...
					<td><?php
						...
						foreach ($options_item['options'] as $key => $option_label) {
							...
							?>
							<input type="radio" name="<?php echo $database_entry; ?>" id="<?php echo $radio_id; ?>"
							       value="<?php echo $key; ?>" <?php checked($radio_setting, $key); ?> />
				...
				<tr>
					...
					<td><?php
						...
						foreach ($options_item['options'] as $check_item) {
							...
							?>
							<input type="checkbox" name="<?php echo $check_nameid; ?>" id="<?php echo $check_nameid; ?>"
							       value="on" <?php echo $checked; ?> />
							<label for="<?php echo $check_nameid; ?>"><?php echo $check_item['desc']; ?></label>
							<br/>
						<?php } ?>

						<input type="hidden" name="<?php echo $checkbox_group_namid; ?>"
						       id="<?php echo $checkbox_group_namid; ?>" value="on"/>
			...
			case 'custom':
				if (is_callable($options_item['options']['callback'])) {
					...
					<tr>
						<td>
							<p>This option is of type "custom", which means it should call a custom function/object
								method defuned by plugin author, but its callback is invalid, it's not callable. Here's
								the option raw data:</p>
							<p>&nbsp;</p>
							<?php
	...
	public function uninstallForm($uninstallArgs)
	{
		...
		<form class="hikari-uninstall-form" method="post" action="">
			<div class="wrap">
				...
				<p>Deactivating <strong><?php echo $uninstallArgs['name']; ?></strong> does not remove any database data
					that may have been created. To completely remove it, you can uninstall it here.</p>
				<p>This option can also be used if you want to reset
					<strong><?php echo $uninstallArgs['name']; ?></strong>'s options to their default.</p>
				<p>&nbsp;</p>
				<p style="color: red"><strong>WARNING:</strong><br/>Once uninstalled, this cannot be undone. You should
					backup your Wordpress database first.</p>
						...
						if (isset($uninstallArgs['options']))
							foreach ($uninstallArgs['options'] as $option) {
								switch ($option['opType']) {
									case 'wp_options':
										echo '<th>WordPress Options</th>';
										break;
									case 'postmeta':
										echo '<th>WordPress Post Metadata</th>';
										break;
									case 'commentmeta':
										echo '<th>WordPress Comments Metadata</th>';
										break;
									case 'tables':
										echo '<th>Tables</th>';
										break;
				...
				<p style="text-align: center;">
					...
					<input type="submit" value="UNINSTALL <?php echo $uninstallArgs['name']; ?>" class="button"
					       onclick="return confirm('You are about to Reset \'<?php echo $uninstallArgs['name']; ?>\' options from WordPress database.\nTHIS ACTION IS NOT REVERSIBLE.\n\n Next page you will be able to verify if options reseting was successiful and finish the uninstalling.\n\n Choose [Cancel] To Stop, [OK] To Reset.')"/>
				</p>
			</div>
		</form>
		<?php
...
// -------------------------------------
	...
	*/
	...
	public function optionsBoxForm($dbGroup, $options_item_index, $opStructure, $opDBName, $opDBValue)
	{
		...
		?>
		<div class="postbox hikari-postbox">
			<div class="inside">
	...
	public function optionsSidebar()
	{
		...
		<div class="hikari-sidebar">
			...
			<div class="widget">
				...
				<ul>
					<li><a href="<?php echo $this->pluginURL; ?>"
					       title="<?php echo $this->optionspageName; ?> page"><?php echo $this->optionspageName; ?>
							page</a></li>
					<?php $this->pluginLinks(); ?>
					<li><a href="http://Hikari.ws/wordpress/" title="A list of all Wordpress plugins I've developed :)">Hikari's
							plugins</a></li>
					<li><a href="http://Hikari.ws/" title="Hikari WebSite">Hikari WebSite</a></li>
					<li><a href="http://Hikari.ws/notas/742/follow-me-on-twitter/" title="Follow me on Twitter">Follow
							me on Twitter</a></li>
				</ul>
			</div>
			...
			<div class="widget">
				...
				<div class="donate">
					...
					<p>But when plugin developers earn money from their fair work, we work harder and try to improve our
						quality.</p>
					<p>&nbsp;</p>
					...
					<form target="_blank" action="https://www.paypal.com/cgi-bin/webscr" method="post">
						<fieldset>
							...
							<input type="image" src="http://img231.imageshack.us/img231/4354/paypaldonate.gif"
							       name="submit" alt="Donate by Paypal" title="Donate by Paypal"/>
						</fieldset>
					</form>
				</div>
			</div>
