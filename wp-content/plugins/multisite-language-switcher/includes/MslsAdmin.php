...
class MslsAdmin extends MslsMain {
	...
	public function main_section() {
        ...
        add_settings_field( 'sort_by_blog_order', __( 'Sort output by blog order', 'multisite-language-switcher' ), array(
            $this,
            'sort_by_blog_order'
        ), __CLASS__, 'main_section' );
        ...
        add_settings_field( 'blog_order', __( 'Blog Order', 'multisite-language-switcher' ), array(
            $this,
            'blog_order'
        ), __CLASS__, 'main_section' );
    ...
    /**
     * Show sort_by_blog_order-field
     *
     * You can decide that the ouput will be sorted by the blog_order. If not
     * the output will be sorted by the language-code.
     */
    public function sort_by_blog_order() {
        echo $this->render_checkbox( 'sort_by_blog_order' );
    }
    ...
    /**
     * The blog_order for the current blog
     *
     * The language will be used ff there is no blog_order.
     */
    public function blog_order() {
        echo $this->render_input( 'blog_order', '2', '0', false, 'number');
    }
	/**
	 ...
	 * @return string
	 */
	public function render_input( $key, $size = '30', $default = '', $readonly = false, $type = 'text' ) {
		return sprintf(
			'<input id="%1$s" name="msls[%1$s]" value="%2$s" size="%3$s"%4$s type="%4$s" />',
			$key,
