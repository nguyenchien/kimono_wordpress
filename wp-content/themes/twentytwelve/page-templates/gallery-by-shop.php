...
/* get field group and child of this - start */
$shopList = array();
$my_field_groups = get_posts(array(
    'name'        => 'acf_customer-gallery',
    'post_type'   => 'acf',
    'post_status' => 'publish',
    'numberposts' => 1
));
if( $my_field_groups ) :
    $fields = array();
    $fields = apply_filters('acf/field_group/get_fields', $fields, $my_field_groups[0]->ID);
    if( $fields ) {
...
/* get field group and child of this - end */
...
$shopId = (int)get_field('shop_id', $parentId );
                                    ...
                                    <section class="wrap-customer-gallery-new">
                                        <div class="wrap-banner-gallery">
                                            <div class="banner-gallery">
                                                ...
                                                if ($isSmartPhone) {
                                                    ?>
                                                    <img src="<?= WP_HOME; ?>/images/new-kimono/banner-shop-gallery-sp-<?= $lang ?>.jpg" width="414" height="205" alt="<?php echo Yii::t('new-kimono-gallery', 'お客様ギャラリー');?>">
                                                    <?php
                                                } else { ?>
                                                    <img src="<?= WP_HOME; ?>/images/new-kimono/banner-shop-gallery-pc-<?= $lang ?>.jpg" alt="<?php echo Yii::t('new-kimono-gallery', 'お客様ギャラリー');?>">
                                                <?php } ?>
                                            </div>
                                            <div class="des-gallery">
                                                <h1 class="title-gallery"><?= get_the_title( $post->post_parent ); ?></h1>
                                                ...
                                                <div class="item-shop item-search">
                                                    <div class="wrap-search-wg-pl flexbox">
                                                        <select name="plan_type" id="plan_type"class="sl-search-shop">
                                                            ...
                                                            <?php if(!empty($planByShop[$shopId])): ?>
                                                                <?php foreach ($planByShop[$shopId][0] as $slug => $planByShopNameKimono) { ?>
                                                                    <option value="<?= $slug?>" class="kimono-plan"><?= Yii::t('new-kimono-gallery-plan',$planByShopNameKimono);?></option>
                                                                <?php } ?>
                                                                <?php foreach ($planByShop[$shopId][1] as $slug => $planByShopNameYukata) { ?>
                                                                    <option value="<?= $slug?>" class="yukata-plan"><?= Yii::t('new-kimono-gallery-plan',$planByShopNameYukata);?></option>
                                                                <?php } ?>
                                                            <?php endif; ?>
                                                        </select>
                                                    </div>
                                        ...
                                        <?php if(get_field('info_bottom_gallery_by_shop')): ?>
                                            <div class="intro-top-general pc"><?php the_field('info_bottom_gallery_by_shop'); ?></div>
                                        <?php endif; ?>
                                    </section>
                                </div>
                            </div>
...
<script>
    ...
    jQuery(function($){
        //Get selected shop's option
        var shopName = '<?php echo $shopListGallery[$shopId][0]; ?>';
        $('#shop_plan option[value="'+ shopName+ '"]').attr('selected','selected');
...
//                }
        ...
        if(pageUrl == "" && planTypeUrl == ""){
            ...
            load_posts(pageUrl, planTypeUrl, shopName);
        }
