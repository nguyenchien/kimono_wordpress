...
<?php
$date = date('Ymd');
wp_enqueue_style( 'header-formal-style' , get_template_directory_uri()  . '/css/header-formal.css', array('twentytwelve-style'),'201802021658');
...
wp_register_style('new-formal-font-icon', get_template_directory_uri() . '/css/icon-formal-rental.css', array('twentytwelve-style'), '201802021121');
...
<div id="page" class="hfeed site">
	...
	<?php
		...
		// Get header by cache
		/*if ($isSmartPhone) {
			$header = Yii::app()->cache->get("header_formal_sp_{$lang}");
		} else {
			$header = Yii::app()->cache->get("header_formal_pc_{$lang}");
		}
		...
		if ($header == false) {
			...
			if ($isSmartPhone) {
				Yii::app()->cache->set("header_formal_sp_{$lang}", $header);
			} else {
				Yii::app()->cache->set("header_formal_pc_{$lang}", $header);
			}
