<?php
	...
	global $isSmartPhone;
?>
<?php if($shortcode['customer_today']['display']==='on' && !$isSmartPhone): ?>

	<?php
	global $custom_post_type, $custom_taxonomies, $post, $blogs, $sites;
...
if($shortcode['news']['display']==='on' && !$isSmartPhone):
	...
	if($the_query -> have_posts()):
		echo '<div class="box-news-top" style="padding-bottom: 10px;">';
				...
				echo '<h2><span class="icon-icon-news"></span><span clas="en">News</span><span class="ja">'.Yii::t('wp_theme', 'お知らせ').'</span></h2>';
				echo '</div>';
				...
				while ($the_query -> have_posts()){
						...
						$attachment_id = get_post_thumbnail_id($post->ID);
						if (!empty($attachment_id)) {
                            swe_the_post_thumbnail($post, $size = array(121,114), $attr = array(
...
//		echo '<p><a class="top-block-news-readmove" href="'. WP_HOME .'/yukata/news" title="'.Yii::t('wp_theme','これまでのニュースを一覧を見る').'">'.Yii::t('wp_theme','これまでのニュースを一覧を見る').'</a></p>';
		echo '<p><a class="top-block-news-readmove" href="'. esc_url(home_url('yukata/news')) .'" title="もっと見る">もっと見る</a></p>';
		echo '</div>';
	endif;
endif;
?>
...
if($shortcode['media']['display']==='on'):
	...
	if($the_query -> have_posts()):
		...
		while ($the_query -> have_posts()){
			...
			$attachment_id = get_post_thumbnail_id($post->ID);
			if (!empty($attachment_id)) {
                swe_the_post_thumbnail($post, $size = array(186,263), $attr = array(
                    //	'src' => $src,
                    'class' => 'attachment-'.implode('x', $size) . ' '.$attachment_id,
                    'id' => '1_1_main',
                    'alt' => strip_tags(get_the_title()),
