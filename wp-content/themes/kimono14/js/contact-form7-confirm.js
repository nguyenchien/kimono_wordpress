...
jQuery(document).ready(function(){
	...
	// laber for
	//jQuery('.wpcf7-list-item-label').on('click', function () { jQuery(this).prev('input[type="radio"').prop('checked', true); });
	jQuery('.wpcf7-list-item-label').on('click', function () { 
		$that = jQuery(this).prev('input[type="radio"'); 
		if(!$that.prop('disabled')) $that.prop('checked', true).change(); 
	});
});
